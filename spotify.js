...
async function displayAlbums() {
    ...
    async function getTopTracks(artistId, accessToken) {
        response = await fetch(`https://api.spotify.com/v1/artists/${artistId}/top-tracks?market=US`, {
            headers: {
        ...
        const data = await response.json();
        console.log(data)
        songs = data.tracks.map(track => track.preview_url).filter(url => url !== null);
...
function playMusic(song, pause = false) {
    ...
    document.querySelector(".songinfo").innerHTML = "Now Playing";
